...
### Build the Application (if necessary):
  ...
  - Run the Application:
      - Execute the currency converter application using the maven or your preferred executable file making sure you use the appropriate command on the terminal (for maven: mvn clean install, mvn compile is run and to rebuild mvn clean compile is used). For Java applications, you might run a command like java -jar <app-name>.jar
        and ensure your dependencies are included in the pom.xml file when built with maven. This ensures your JSON object are compiled with the rest of the code.
